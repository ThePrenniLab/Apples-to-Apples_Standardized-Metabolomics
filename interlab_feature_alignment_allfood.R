...
available_data <- combined_data  # make a copy that shrinks
...
for (i in 1:nrow(unique_values)) {
  ...
  # Find matches within the available pool
  matches <- available_data %>%
  ...
  if (nrow(matches) > 0) {
    ...
    # Remove matched IDs from future pool
    available_data <- available_data %>%
      filter(!(ID %in% top_matches$ID))
